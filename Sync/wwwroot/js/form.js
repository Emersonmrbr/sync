...
function handleErrorState(parent, label, errorElement, condition) {
...
function handleNameOrLastnameEvent(event, parent) {
  ...
  handleErrorState(parent, label, error, (size < 3 || size > 50) && size !== 0);
}
...
function handlePhoneEvent(event, parent) {
  ...
  handleErrorState(parent, label, error, isError);
}
...
function handleEmailEvent(event, parent) {
  ...
  const re = /\S+@\S+\.\S+/;
  const error = parent.querySelector(`[js-error]`);
  handleErrorState(parent, label, error, !re.test(dataInput));
}
